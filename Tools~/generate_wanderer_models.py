...
OUT = Path(sys.argv[1]) if len(sys.argv) == 2 else Path("public/assets/models")
...
def make_scout():
    ...
    cone(a, "spear_tip", 0.11, 0.23, "stone", (0.67, 0.55, 0), sides=5)
...
def make_brute():
    ...
    ico(a, "club_head", 0.27, "bronze", (1.04, 0.92, 0.05), scale=(1.3, 0.85, 1.0))
...
def make_archer():
    ...
    box(a, "bow", (0.09, 0.95, 0.09), "wood", (0.55, 0.6, 0), rotate_y=0.15)
...
def main():
    make_projectile_knight()
    make_projectile_wizard()
    make_projectile_archer()
    make_scout()
    make_brute()
    make_spitter()
    make_elite()
    ...
    make_storage()
    make_healing_hut()
    print(f"generated {len(list(OUT.glob('*.glb')))} GLB files in {OUT}")
