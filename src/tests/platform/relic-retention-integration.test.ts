...
import { meshFor, rendererDom, rendererSnapshot } from "./renderer-test-helpers";
...
const relicFrame = () => {
  ...
  return {
    ...snapshot,
    weaponRelicDrops: [
      { id: "relic:one", position: { x: 3, y: 4 }, waveIndex: 1, bossName: "Fixture boss" },
      { id: "relic:two", position: { x: -2, y: 5 }, waveIndex: 2, bossName: "Fixture boss" },
    ],
    projectiles: [
      { ...snapshot.projectiles[0], id: "homing", style: "magic" as const, homing: true },
      { ...snapshot.projectiles[0], id: "ordinary", style: "magic" as const, homing: false },
    ],
  };
};
...
describe("released relic visuals in the retained projection", () => {
  it("keeps relic and homing canvas diagnostics current", () => {
    ...
    renderer.render({ ...frame, weaponRelicDrops: [], projectiles: frame.projectiles.map(projectile => ({ ...projectile, homing: false })) });
  ...
  it("preserves exact released relic/homing shape and material with shared resources", () => {
    ...
    expect((relic.geometry as THREE.OctahedronGeometry).parameters).toMatchObject({ radius: 0.34, detail: 0 });
    ...
    expect(material).toMatchObject({ emissiveIntensity: 1.1, roughness: 0.2, metalness: 0.55 });
    const homing = meshFor(projection, "homing");
    expect((homing.geometry as THREE.SphereGeometry).parameters.radius).toBe(0.28);
    expect((homing.material as THREE.MeshStandardMaterial).color.getHex()).toBe(0xe6d6ff);
    expect((homing.material as THREE.MeshStandardMaterial).emissive.getHex()).toBe(0xab78ff);
    expect((meshFor(projection, "ordinary").geometry as THREE.SphereGeometry).parameters.radius).toBe(0.23);
    projection.dispose();
  });
  ...
  it("retains identical frames, updates homing in place, removes relics and disposes once", () => {
    ...
    for (let index = 0; index < 50; index += 1) projection.render(structuredClone(frame));
    expect(projection.diagnostics()).toEqual(before);
    projection.render({ ...frame, projectiles: frame.projectiles.map(projectile => ({ ...projectile, homing: false })) });
    ...
    expect(projection.diagnostics().meshesRemoved).toBe(before.meshesRemoved + 2);
