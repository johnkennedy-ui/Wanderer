...
test("environment clones load through the existing model cache at the deployment base", async ({
  page,
}, testInfo) => {
...
test("missing environment models retain obstacle fallback without retry loops or input interception", async ({
  page,
}) => {
  const faults = captureBrowserFaults(page);
  ...
  let requests = 0;
  await page.route(
    "**/assets/models/expansion-v1/rock_*.glb",
    async (route) => {
      ...
      await route.fulfill({
        ...
        body: "missing environment fixture",
      });
    },
  );
  await primeModelWorld(page, "knight");
  ...
  await expect(canvas).toHaveAttribute("data-environment-pending-count", "0");
  await expect(canvas).toHaveAttribute(
    "data-environment-fallback-count",
    /[1-9]\d*/,
  );
  ...
  for (const fault of faults) {
    expect(
      [...missing].some((url) => fault.startsWith(`console: ${url} `)),
    ).toBe(true);
